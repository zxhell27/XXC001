-- TurtleSpy V1.5.4, credits to Intrer#0421, updated to support UnreliableRemoteEvents
...
local unreliableImage = "http://www.roblox.com/asset/?id=6035041212" -- Ikon contoh untuk UnreliableRemoteEvent
local remotes = {}
local remoteArgs = {}
local remoteButtons = {}
local remoteScripts = {}
...
ImageButton.MouseButton1Click:Connect(function()
    -- Reset browsed remotes, connections, and offsets when opening the browser
    for _, conn in pairs(browsedConnections) do
        conn:Disconnect()
    end
    browsedConnections = {}
    for _, btn in pairs(RemoteBrowserFrame:GetChildren()) do
        if btn:IsA("TextButton") and btn.Name == "RemoteButton" then
            btn:Destroy()
        end
    end
    browsedButtonOffset = 10
    browserCanvasSize = 286 -- Reset ke ukuran awal
    RemoteBrowserFrame.CanvasSize = UDim2.new(0, 0, 0, browserCanvasSize) -- Set ulang canvas size

    BrowserHeader.Visible = not BrowserHeader.Visible
    if BrowserHeader.Visible then -- Hanya populate jika browser terlihat
        for i, v in pairs(game:GetDescendants()) do
            if isA(v, "RemoteEvent") or isA(v, "RemoteFunction") or isA(v, "UnreliableRemoteEvent") then
                ...
                -- Perbaikan: Pastikan canvas size diperbarui dengan benar
                if browsedButtonOffset + 35 > browserCanvasSize then -- Jika offset melebihi canvas size saat ini
                    browserCanvasSize = browsedButtonOffset + 35 -- Sesuaikan canvas size
...
CodeComment.Text = "-- Script generated by TurtleSpy, Upgrade by Z3DLI5T"
...
local function updateInfoButtonsScrollCanvasSize()
    ...
    -- Tambahkan padding untuk ArgumentsDisplay
    maxHeight = maxHeight + ArgumentsDisplay.Size.Y.Offset + 20 -- 20 adalah padding tambahan
...
ViewArgs.MouseButton1Click:Connect(function()
    ...
    updateInfoButtonsScrollCanvasSize() -- Perbarui canvas size saat ArgumentsDisplay diubah
end)
...
CloseInfoFrame.MouseButton1Click:Connect(function()
    ...
    InfoFrameOpen = false
    mainFrame.Size = UDim2.new(0, 207, 0, 35)
    ArgumentsDisplay.Visible = false -- Sembunyikan ArgumentsDisplay saat InfoFrame ditutup
end)
...
OpenInfoFrame.MouseButton1Click:Connect(function()
    ...
    if not InfoFrame.Visible then
        ArgumentsDisplay.Visible = false -- Sembunyikan ArgumentsDisplay saat InfoFrame ditutup via OpenInfoFrame
    end
end)
...
Minimize.MouseButton1Click:Connect(function()
    if RemoteScrollFrame.Visible then
        ...
        OpenInfoFrame.Text = "<"
        InfoFrame.Visible = false
        ArgumentsDisplay.Visible = false -- Sembunyikan ArgumentsDisplay saat diminimalkan
    else
...
local function FindRemote(remote, args)
    ...
    if table.find(unstacked, remote) then
        -- Jika remote di-unstack, kita cari berdasarkan remote dan argumen
        for b, v in pairs(remotes) do
            if v == remote then
                -- Periksa apakah argumennya cocok
                local argsMatch = true
                if #remoteArgs[b] ~= #args then
                    argsMatch = false
                else
                    for k, val in pairs(args) do
                        if remoteArgs[b][k] ~= val then
                            argsMatch = false
                            break
                        end
                    end
...
Clear.MouseButton1Click:Connect(function()
    for i, v in pairs(RemoteScrollFrame:GetChildren()) do
        if v.Name == "RemoteButton" then -- Pastikan hanya menghapus RemoteButton
            v:Destroy()
        end
    end
    ...
    buttonOffset = -25
    scrollSizeOffset = 287 -- Reset ke nilai awal yang benar
    remotes = {}
    remoteArgs = {}
    remoteButtons = {}
    remoteScripts = {}
    ...
    connections = {}
    unstacked = {}
    lookingAt = nil -- Reset lookingAt
    lookingAtArgs = nil -- Reset lookingAtArgs
    ArgumentsDisplay.Visible = false -- Sembunyikan ArgumentsDisplay saat membersihkan log
    RemoteScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 287)
...
DoNotStack.MouseButton1Click:Connect(function()
    if lookingAt then
        ...
        if isUnstacked then
            ...
            -- Memerlukan penataan ulang tombol jika remote ini sebelumnya di-unstack dan sekarang di-stack
            -- Untuk menyederhanakan, kita bisa memicu clear dan rebuild (ini akan memakan sedikit lebih banyak performa tapi lebih mudah)
            -- Atau, kita bisa menghapus tombol-tombol yang berhubungan dengan remote ini dan menambahkannya kembali sebagai stacked.
            -- Karena 'Clear' sudah ada, kita bisa memanfaatkannya untuk mereset tampilan secara efektif.
            -- Namun, untuk menghindari clear semua, kita akan menghapus dan menambahkan kembali.
            local tempRemote = lookingAt
            local tempArgs = lookingAtArgs
            
            -- Hapus instance remote yang tidak ditumpuk dari daftar dan GUI
            for i = #remotes, 1, -1 do
                if remotes[i] == tempRemote then
                    remoteButtons[i].Parent:Destroy()
                    table.remove(remotes, i)
                    table.remove(remoteArgs, i)
                    table.remove(remoteButtons, i)
                    table.remove(remoteScripts, i)
                end
            end

            -- Reset posisi tombol dan ukuran scroll
            buttonOffset = -25
            scrollSizeOffset = 287
            RemoteScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 287)

            -- Tambahkan kembali remote yang relevan, sekarang sebagai stacked
            for i, remote_val in pairs(remotes) do
                -- Ini akan memicu ChildAdded dan membuat tombol baru
                addToList(isA(remote_val, "UnreliableRemoteEvent") and "unreliable" or isA(remote_val, "RemoteEvent"), remote_val, unpack(remoteArgs[i]))
            end
            -- Perbarui tombol tampilan untuk remote yang dipilih saat ini (jika masih ada)
            lookingAt = tempRemote
            lookingAtArgs = tempArgs
            local newIndex = FindRemote(lookingAt, lookingAtArgs)
            if newIndex then
                lookingAtButton = remoteButtons[newIndex]
            else
                lookingAt = nil
                lookingAtArgs = nil
                lookingAtButton = nil
            ...
            -- Memerlukan penataan ulang tombol agar remote yang sama dengan argumen berbeda muncul sebagai entri terpisah
            -- Ini akan memicu clear dan rebuild agar tampilan diperbarui dengan benar
            local tempRemote = lookingAt
            local tempArgs = lookingAtArgs
            Clear.MouseButton1Click:Connect(function() end)() -- Panggil Clear (tanpa listener) untuk membersihkan log dan GUI
            -- Kemudian, tambahkan kembali semua remote, termasuk yang sekarang di-unstack
            -- Ini akan memerlukan iterasi melalui semua koneksi dan memanggil addToList lagi
            -- Alternatif yang lebih sederhana adalah hanya memperbarui tampilan saat remote itu sendiri terdeteksi lagi
            -- Untuk saat ini, asumsikan remote akan di-unstack saat berikutnya terdeteksi.
            -- Logika yang lebih kompleks akan melibatkan pembuatan tombol baru untuk setiap argumen yang berbeda.
        end
    end
end)
...
RemoteScrollFrame.ChildAdded:Connect(function(child)
    ...
    local connection = child.MouseButton1Click:Connect(function()
        ...
        -- Perbaikan: Panggil updateInfoButtonsScrollCanvasSize() untuk penyesuaian dinamis
        -- Tidak perlu UDim2.new(0, 0, 1.1, 0) atau 1, biarkan fungsi yang menanganinya
        updateInfoButtonsScrollCanvasSize() 

        mainFrame.Size = UDim2.new(0, 565, 0, 35)
        OpenInfoFrame.Text = ">"
        InfoFrame.Visible = true
        ArgumentsDisplay.Visible = false -- Sembunyikan ArgumentsDisplay saat remote baru dipilih
        ...
        -- Perbarui status tombol DoNotStack saat remote baru dipilih
        local isUnstacked = table.find(unstacked, lookingAt)
        if isUnstacked then
            DoNotStack.Text = "Stack remote"
            DoNotStack.TextColor3 = Color3.fromRGB(251, 197, 49)
...
function addToList(type, remote, ...)
    ...
    if not i then
        ...
        if #remotes * 35 > scrollSizeOffset then -- Perbaikan: Sesuaikan scrollSizeOffset secara dinamis
            scrollSizeOffset = #remotes * 35 + 287 -- Tambahkan padding awal
...
OldEvent = hookfunction(Instance.new("RemoteEvent").FireServer, function(Self, ...)
    ...
    addToList(true, Self, ...)
    return OldEvent(Self, ...) -- Pastikan hookfunction mengembalikan panggilan asli
end)
...
OldFunction = hookfunction(Instance.new("RemoteFunction").InvokeServer, function(Self, ...)
    ...
    return OldFunction(Self, ...) -- Pastikan hookfunction mengembalikan panggilan asli
end)
