...
ClearBorderColor3 = colorSettings["MainButtons"]["BorderColor"]
...
ArgsFrame.Position = UDim2.new(0.0391303748, 0, 0.236, 0) -- Di bawah CodeFrame
...
ArgsText.ZIndex = 18
ArgsText.Font = Enum.Font.SourceSans
...
local function updateInfoButtonsScrollCanvas()
    ...
    local canvasHeight = buttonCount * 35 + 10 -- 35 per tombol + padding
    InfoButtonsScroll.CanvasSize = UDim2.new(0, 0, 0, canvasHeight)
end

-- Panggil fungsi saat InfoFrame dibuka atau tombol ditambahkan
InfoButtonsScroll.ChildAdded:Connect(updateInfoButtonsScrollCanvas)

-- Tambahan: Logika untuk ViewArgs
ViewArgs.MouseButton1Click:Connect(function()
    if lookingAt and lookingAtArgs then
        ...
        setclipboard(argsString) -- Salin ke clipboard juga
...
RemoteScrollFrame.ChildAdded:Connect(function(child)
    ...
    local connection = child.MouseButton1Click:Connect(function()
        ...
        -- Perbarui CanvasSize berdasarkan tipe remote
        ...
        InfoFrameOpen = true
        ArgsText.Text = "Click 'View Arguments' to display args" -- Reset ArgsText
    end)
...
-- Panggil fungsi untuk mengatur CanvasSize awal
updateInfoButtonsScrollCanvas()
